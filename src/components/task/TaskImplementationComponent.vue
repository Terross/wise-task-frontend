                        ...
                        <v-list lines="one">
                            ...
                            >
                                <template v-slot:prepend>
                                    <v-chip class="ma-2" v-if="item.originalResult === item.result" color="success">
                                        {{i + 1}}
                                    </v-chip>
...
export default defineComponent({
    ...
    methods: {
        ...
        async solveTask() {
            const { mutate, onDone, onError } = useMutation(SOLVE_TASK_IMPLEMENTATION)
            const jarFile = ((await this.toBase64(this.jarFileModel[0])) as string).replace("data:application/x-java-archive;base64,", "")
            const request = {
                solution: {
